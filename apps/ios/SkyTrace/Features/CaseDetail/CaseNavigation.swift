...
struct PaywallContext: Identifiable {
    enum Trigger: String { case briefing, synthesis, filters, tracking, summary, socialReports }
    let id = UUID()
    let trigger: Trigger
...
private extension PaywallContext.Trigger {
    static let ordered: [PaywallContext.Trigger] = [.summary, .synthesis, .briefing, .filters, .tracking, .socialReports]
}
