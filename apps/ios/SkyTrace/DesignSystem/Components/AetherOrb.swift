...
struct AetherOrb: View {
    ...
    private func draw(_ ctx: inout GraphicsContext, _ size: CGSize) {
        ...
        // 1. Outer halo bloom — the object's glow into the night.
        let bloom = Path(ellipseIn: CGRect(x: center.x - r * 2.4, y: center.y - r * 2.4,
                                           width: r * 4.8, height: r * 4.8))
        ctx.fill(bloom, with: .radialGradient(
            Gradient(colors: [SkyColor.aetherGlow.opacity(0.16 * halo), .clear]),
            center: center, startRadius: r * 0.6, endRadius: r * 2.4))

        // 2. Concentric orbit rings — perspective ellipses give depth.
        ...
        // 6. Deterministic star specks around the field.
        for i in 0..<7 {
            let a = Double((seed * 47 + i * 51) % 360) * .pi / 180
            let dist = r * (1.5 + Double((seed + i * 13) % 7) / 7.0)
        ...
        //    lens, with a soft halo. Parallax makes it float above the sphere.
        let orbitR = r * 1.62
        let t = (2 * Double.pi) * phase + Double(seed % 360) * .pi / 180
        let lp = CGPoint(x: center.x + cos(t) * orbitR + par.width * 1.6,
                         y: center.y + sin(t) * orbitR * 0.62 + par.height * 1.6)
        ctx.fill(Path(ellipseIn: CGRect(x: lp.x - r * 0.7, y: lp.y - r * 0.7,
                                        width: r * 1.4, height: r * 1.4)),
                 with: .radialGradient(
                    Gradient(colors: [SkyColor.statusNew.opacity(0.5 * halo), .clear]),
                    center: lp, startRadius: 0, endRadius: r * 0.7))
    ...
    private var ringSpecs: [RingSpec] {
        [.init(scale: 1.28, color: SkyColor.accentPrimary, opacity: 0.55),
         .init(scale: 1.62, color: SkyColor.auroraCyan, opacity: 0.32),
         .init(scale: 2.02, color: SkyColor.auroraViolet, opacity: 0.20)]
    }
}
