...
## News First / AI Second（SKYTRACE_NEWS_FIRST_PRODUCTION_DIRECTIVE）

- **D-NF-001 既存ドメインモデルを再利用し、並行モデルを作らない**: 指示書は `NewsArticle`/`PrimarySource`/`PremiumSummary` 等の新規struct候補を挙げていたが、既存の `SourceReference`/`MediaAsset`/`ExplanationCandidate` 等は既にNews First要件（媒体名・種別・権利ゲート・役割）を満たしていたため、`Domain/Models/NewsFirstPresentation.swift` に **既存モデルからの導出プロパティ**（`primarySources`/`primaryMedia`/`premiumSummary`/`leadOutletName`等）として実装。Fixture・Repository・既存テストへの影響を最小化しつつ、9個のfixture caseを無改変で新UIに流用できる。
- **D-NF-002 CaseDetailV2Viewの並び順を固定**: NewsCaseHeader → PrimaryNewsMediaView → PrimarySourceActions → PremiumSummarySection → 確認できること/出典/更新履歴 → `AIReferenceDisclosure`（既定閉）→ `DetailedAssessmentDisclosure`（既定閉）→ 関連事例。旧「現時点の判断」(`CaseExecutiveSummary`)はAI補足の折りたたみ内へ格下げ（AIが判定する画面に見えないようにするため）。
- **D-NF-003 AI補足・詳細評価はDisclosureGroupで既定`false`**: `ReferenceDisclosures.swift` に共通実装 `CollapsibleReferenceSection` を用意し、`AIReferenceDisclosure`/`DetailedAssessmentDisclosure` の両方が`@State private var isExpanded = false`から開始。`assess.sectionTitle`キーを「現在の評価」から「詳しい確認データ」へ転用（旧V1 `CaseDetailView`のみが読んでいた値だが、V1は非到達のため実害なし）。
- **D-NF-004 CaseCardは全variantでメディアを先頭に**: `featured`/`standard`/`compact`/`mapSheet`すべてで`primaryMedia`（権利確認済みのみインライン、なければ`ObservationGlyph`）を最上部に配置。DEMOバッジは画像上のスクリム付きバッジへ移動（視認性維持）。8軸評価・AI結論はカードに一切出さない方針は既存のまま。
